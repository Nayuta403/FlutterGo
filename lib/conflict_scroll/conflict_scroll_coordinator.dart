...
import 'conflict_scroll_controller.dart';
import 'conflict_scroll_position.dart';

/// Copyright 2020 ke.com. All rights reserved.
/// @author 俊达
/// @date   2020/8/10 5:35 PM
/// @desc   协调器
enum PageExpandState { NotExpand, Expanding, Expanded }

class ConflictScrollCoordinator {
  /// 页面主 CustomScrollView 控制
  final String pageLabel = "page";

  ConflictScrollController _pageScrollController;
  double Function() pinnedHeaderSliverHeightBuilder;

  ConflictScrollPosition get _pageScrollPosition =>
  ...
  /// 获取主页面滑动控制器
  ConflictScrollController pageScrollController([double initialOffset = 0.0]) {
    assert(initialOffset != null, initialOffset >= 0.0);
    _pageInitialOffset = initialOffset;
    _pageScrollController = ConflictScrollController(this,
        debugLabel: pageLabel, initialScrollOffset: initialOffset);
    return _pageScrollController;
  }

  /// 创建并获取一个子滑动控制器
  ConflictScrollController newChildScrollController([String debugLabel]) =>
      ConflictScrollController(this, debugLabel: debugLabel);
  ...
  bool applyContentDimensions(double minScrollExtent, double maxScrollExtent,
      ConflictScrollPosition position) {
  ...
  void updateUserScrollDirection(
      ConflictScrollPosition position, ScrollDirection value) {
